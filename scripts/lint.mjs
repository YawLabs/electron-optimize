 ...
 * in well under a second once warm (31-564ms measured across runs under x64
 * emulation on Windows ARM64; the first cold run reported 1.3s).
 */
 ...
 * 2.4.11 and the lockfile at 2.5.6, the emulated binary linted with a version
  ...
  if (install.status !== 0 || !existsSync(bin)) {
    ...
    throw new Error(
      `Failed to provision @biomejs/cli-win32-x64@${version} (${why}).\n` +
        "Actions is disabled on this repo, so there is no other lint signal. Fix the\n" +
        "install, or set YAWLABS_BIOME_BIN=<path to a working biome> to point this script\n" +
        "at one.",
    );
  }
  return bin;
